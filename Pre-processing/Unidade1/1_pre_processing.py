from src.utils import load_volunteer_dataset

volunteer = load_volunteer_dataset()

# Mostre a dimensão do dataset volunteer
print("dimensão dataset volunteer:")
print(volunteer.shape, "\n")

#mostre os tipos de dados existentes no dataset
print("tipos de dados existentes no dataset:")
print(volunteer.info(), "\n")

#mostre quantos elementos do dataset estao faltando na coluna
print("quantos elementos do dataset estao faltando na coluna locality:")
print(volunteer["locality"].isna().sum(), "\n")


# Exclua as colunas Latitude e Longitude de volunteer
print("excluido as colunas Latitude e Longitude de volunteer:")
volunteer_cols = volunteer.drop(["Latitude", "Longitude"], axis = 1)
print(volunteer_cols, "\n")

# Exclua as linhas com valores null da coluna category_desc de volunteer_cols
print("excluido as linhas com valores null da coluna category_desc de volunteer_cols:")
volunteer_subset = volunteer_cols.dropna(subset=["category_desc"])
print(volunteer_subset, "\n")

# Print o shape do subset
print("shape do subset:")
print(volunteer_subset.shape)

